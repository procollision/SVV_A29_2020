...
def I_zz(h_a, C_a, t_sk, h_st, w_st, t_st, N_st, t_sp, st_cord, cg):
    ...
    I_zz = []
    # [CHECK] Defines the list for the z coordinates of the stringers and the z location of the cg.
    ...
    I_sk = 1/12*t_sk*L_sk*(np.sin(phi))**2
    I_zz += 2*[I_sk]
    print(I_sk)
...
def I_yy(h_a, C_a, t_sk, h_st, w_st, t_st, N_st, t_sp, st_cord, cg):
    ...
    A_sc = np.pi*((0.5*h_a)**2-(0.5*h_a-t_sk)**2)
    S_sc = A_sc*dy_sc**2
    I_yy.append(S_sc)
    ...
    phi = np.arctan((0.5*h_a)/(C_a - 0.5*h_a))
    L_sk = ((0.5*h_a)**2+(C_a-(0.5*h_a))**2)**0.5
    I_sk = 1/12*t_sk*L_sk*(np.cos(phi))**2
    I_yy += 2*[I_sk]
    
    # Calculate the Steiner term of the skin elements. 
    A_sk = L_sk*t_sk
    cg_zsk = (C_a-(0.5*h_a)) - 0.5*L_sk*np.cos(phi)
